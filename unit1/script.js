...
function updateGalaxyStatus() {
    ...
    Object.keys(galaxyConfigs).forEach(galaxyId => {
        ...
        if (statusElement) {
            const galaxy = galaxyConfigs[galaxyId];
            const card = statusElement.closest('.galaxy-card');
...
function updateShopDisplay() {
    ...
    Object.keys(shopItems).forEach(itemId => {
        ...
        const button = document.querySelector(`[onclick="buyUpgrade('${itemId}')"]`);
        ...
        if (button) {
            if (currentLevel >= item.maxLevel) {
                ...
                button.disabled = playerData.killCoins < cost;
            }
        }
    });
}
...
function draw() {
    ...
    if (gameState.gameOver) {
        ...
        ctx.fillStyle = '#ffff00';
        ctx.font = '20px Arial';
        ctx.fillText(`Final Score: ${gameState.score}`, GAME_WIDTH / 2, GAME_HEIGHT / 2 + 40);
    }
}
